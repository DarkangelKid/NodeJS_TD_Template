...
function CASAuthentication(options) {
  ...
  this.session_info =
    ['2.0', '3.0', 'saml1.1'].indexOf(this.cas_version) >= 0 && options.session_info !== undefined ? options.session_info : false;
...
CASAuthentication.prototype._login = function (req, res, next) {
  ...
  console.log('Redirect to the CAS login');
  console.log(
    this.cas_url +
      url.format({
        pathname: '/login',
          ...
          this._validate(
            ...
            function (err, user, attributes) {
              if (err) {
                ...
                res.redirect(req.session.cas_return_to + '/loginwithtoken?token=' + user);
              }
            }.bind(this),
          );
        }.bind(this),
